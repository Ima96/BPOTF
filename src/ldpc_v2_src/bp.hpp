...
namespace ldpc {
   ...
   public:
               ...
               iterations(0) 
      {
         ...
         this->compute_initial_log_prob_ratios();
      ...
      void compute_initial_log_prob_ratios()
      {
         for (uint32_t u32_i = 0U; u32_i < this->bit_count; ++u32_i)
         {
            this->initial_log_prob_ratios[u32_i] =
               std::log((1 - this->channel_probabilities[u32_i]) / this->channel_probabilities[u32_i]);
         }
      }
      ...
      void initialise_log_domain_bp() 
      {
         ...
         {
            // this->initial_log_prob_ratios[i] = 
            //    std::log((1 - this->channel_probabilities[i]) / this->channel_probabilities[i]);
      ...
      void update_channel_probs(std::vector<double> const & vec_f64_new_probs)
      {
         this->channel_probabilities = vec_f64_new_probs;
         // Compute and Re-initialise log domain
         this->compute_initial_log_prob_ratios();
         // this->initialise_log_domain_bp();
      }
      ...
      std::vector<uint8_t> &bp_decode_serial(std::vector<uint8_t> &syndrome)
      {
         ...
         for (int it = 1; it <= maximum_iterations; it++) 
         {
            ...
            {
               double temp = NAN;
               this->log_prob_ratios[bit_index] = 
                  std::log((1 - channel_probabilities[bit_index]) / channel_probabilities[bit_index]);
               if (this->bp_method == 0) 
               {
      ...
      std::vector<uint8_t> & soft_info_decode_serial(std::vector<double> &soft_info_syndrome, 
                        ...
                        if (check_node_sgn == syndrome[check_index]) 
                        {
                           if (std::abs(check_nbr.bit_to_check_msg) < min_bit_to_check_msg) 
                           {
                              this->soft_syndrome[check_index] =
                                 pow(-1, syndrome[check_index]) * std::abs(check_nbr.bit_to_check_msg);
                           } 
                           else 
                           {
                              this->soft_syndrome[check_index] =
